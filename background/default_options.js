...
const DEFAULT_OPTIONS = {
    "keyMapping": {
        ...
        "hint": {
            ...
            "gv": "invokeCommand|0|compose|selectElement|toVisualMode",
...
function convertHintKeyMapping(keyMapping) {
    Object.keys(keyMapping).forEach((key) => {
        ...
        if (cmd.command.startsWith("hint.")) {
            keyMapping[key] = cmd.command.substr(5);
        }
        else {
            keyMapping[key] = `invokeCommand|${cmd.count || 0}|${cmd.command}`;
        }
    });
    return keyMapping;
}
